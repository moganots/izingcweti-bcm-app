<template>
  <q-card flat bordered>
    <q-card-section>
      <div class="text-h6 q-mb-md">Risk Matrix</div>
      <div class="risk-matrix">
        <div class="matrix-grid">
          <!-- Corner -->
          <div class="matrix-cell matrix-header matrix-corner">
            <span class="text-caption">Impact ↓<br />Likelihood →</span>
          </div>
          <!-- Likelihood Headers -->
          <div
            class="matrix-cell matrix-header"
            v-for="likelihood in likelihoods"
            :key="likelihood.label"
          >
            <div class="text-caption font-bold">{{ likelihood.label }}</div>
            <div class="text-caption">{{ likelihood.value }}</div>
          </div>
          <!-- Impact Rows -->
          <template v-for="(impact, i) in impacts" :key="impact">
            <div class="matrix-cell matrix-header matrix-impact">
              <div class="text-caption font-bold">{{ impact }}</div>
            </div>
            <div
              v-for="(likelihood, j) in likelihoods"
              :key="`${i}-${j}`"
              class="matrix-cell matrix-data"
              :class="getCellClass(i, j)"
              @click="$emit('cell-click', { impact, likelihood: likelihood.value })"
            >
              <div class="cell-score">{{ getCellScore(i, j) }}</div>
              <div class="cell-count" v-if="getCellCount(i, j) > 0">
                {{ getCellCount(i, j) }}
              </div>
            </div>
          </template>
        </div>
        <!-- Legend -->
        <div class="matrix-legend q-mt-md">
          <div class="row q-gutter-sm items-center justify-center">
            <q-badge color="green-3" text-color="dark" label="Low (1-5)" />
            <q-badge color="yellow-3" text-color="dark" label="Medium (6-10)" />
            <q-badge color="orange-3" text-color="dark" label="High (11-15)" />
            <q-badge color="red-3" text-color="dark" label="Critical (16-25)" />
          </div>
        </div>
      </div>
    </q-card-section>
  </q-card>
</template>

<script setup lang="ts">
import { computed } from 'vue'

const props = defineProps<{ risks?: any[] }>()
defineEmits<{ 'cell-click': [cell: { impact: string; likelihood: number }] }>()

const impacts = ['Insignificant', 'Low', 'Medium', 'High', 'Critical']
const likelihoods = [
  { label: 'VL', value: 0.2 },
  { label: 'L', value: 0.4 },
  { label: 'M', value: 0.6 },
  { label: 'H', value: 0.8 },
  { label: 'VH', value: 1.0 },
]

const risksData = computed(() => props.risks || [])

function getCellScore(i: number, j: number): number {
  return (i + 1) * (j + 1)
}

function getCellCount(i: number, j: number): number {
  return risksData.value.filter(
    (r: any) =>
      r.impact_severity === impacts[i] && Math.abs(r.likelihood - likelihoods[j]!?.value) < 0.2
  ).length
}

function getCellClass(i: number, j: number): string {
  const score = getCellScore(i, j)
  if (score > 15) return 'cell-critical'
  if (score > 10) return 'cell-high'
  if (score > 5) return 'cell-medium'
  return 'cell-low'
}
</script>

<style lang="scss" scoped>
.matrix-grid {
  display: grid;
  grid-template-columns: 70px repeat(5, 1fr);
  gap: 2px;
}
.matrix-cell {
  padding: 6px 4px;
  text-align: center;
  min-height: 45px;
  display: flex;
  flex-direction: column;
  align-items: center;
  justify-content: center;
}
.matrix-header {
  background: #f5f5f5;
  font-size: 10px;
}
.matrix-corner {
  grid-column: 1;
  grid-row: 1;
}
.matrix-impact {
  grid-column: 1;
}
.matrix-data {
  cursor: pointer;
  transition: transform 0.2s;
  font-size: 12px;
  &:hover {
    transform: scale(1.05);
  }
}
.cell-score {
  font-weight: bold;
  font-size: 13px;
}
.cell-count {
  font-size: 10px;
  opacity: 0.7;
}
.cell-low {
  background: #c8e6c9;
}
.cell-medium {
  background: #fff9c4;
}
.cell-high {
  background: #ffe0b2;
}
.cell-critical {
  background: #ffcdd2;
}
</style>
