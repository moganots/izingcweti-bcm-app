...
export function useDashboard(options: UseDashboardOptions = {}) {
    ...
    const criticalIncidentsCount = computed(() => {
        return recentIncidents.value?.filter((i: any) => i.incidentSeverity === 'Critical').length || 0
    })

    const highPriorityWorkflowsCount = computed(() => {
        return pendingWorkflows.value?.filter((w: any) => (w.priority || 5) <= 2).length || 0
    })

    const overdueTestsCount = computed(() => {
        return upcomingTests.value?.filter((t: any) => t.date && new Date(t.date) < new Date()).length || 0
    })
    ...
    onMounted(() => {
        ...
        // Return cleanup for Vue's onMounted cleanup
        // This works in Vue 3 with onMounted returning cleanup
        if (typeof window !== 'undefined') {
            // For Vue 3, we can use onBeforeUnmount
